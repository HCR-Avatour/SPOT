...
VELOCITY_FACTOR = 0.4
VELOCITY_BASE_SPEED = 0.5*VELOCITY_FACTOR  # m/s
VELOCITY_BASE_ANGULAR = 0.8*VELOCITY_FACTOR  # rad/sec
...
"""
Taken from Boston Dynamics SDK example: xbox_controller.py -> renamed to 
"""
class JoystickController:
    """JoystickController class provides mapping between xbox controller commands and Spot API calls.
    ...
    def __init__(self):
        self.client_name = 'JoystickControllerClient'
        self.robot = None
...
| Back               | Exit                     |
    ...
    def _walk(self):
        ...
        if self.mode is not RobotMode.Walk:
            ...
            self.mobility_params = spot_command_pb2.MobilityParams(
                locomotion_hint=spot_command_pb2.HINT_SPEED_SELECT_TROT, stair_hint=1)
    ...
    def control_robot(self, frequency, joy):
        ...
        try:
         #   joy = XboxJoystickFactory.get_joystick()##ALEERA: left_bumper, right_bumper, B, dpad_up/down/left/right, start, close, back, left_x/y, right_x/y, Y, A, B, X
            #joy = 
            # while not joy.back():###
                #if joy.start():
                if not joy.has_started:
                    self._gain_control()
                    self._power_motors()
                    joy.has_started = True
                    rospy.loginfo("Joystick started and acquired lease")
                else:
                    ...
                    if joy.mode == "standing" and joy.left_y !=0:
                        self._change_height(joy.left_y)
                    ...
                    #         self._amble()
                    #     else:
                    #         self._walk()#WALK MODE
                    # if joy.B() and not joy.right_bumper():
                    #     if joy.left_bumper():
                    #         self._crawl()
                    #     else:
                    #         self._stand()#STAND MODE
                    # if joy.X():
                    #     if joy.left_bumper():
                    #         self._hop()
                    #     else:
...
class spot_receiver:
    def __init__(self, options):
        self.sub_VR_joy_left = rospy.Subscriber("/VR/joy_left", sensor_msgs.msg.Joy, self.call_back_VR_joy_left, queue_size=1)
        self.sub_VR_joy_right = rospy.Subscriber("/VR/joy_right", sensor_msgs.msg.Joy, self.call_back_VR_joy_right, queue_size=1)
        self.sub_VR_joy_mode = rospy.Subscriber("/VR/joy_mode", std_msgs.msg.String, self.call_back_VR_mode, queue_size=1)
        self.sub_VR_joy_control = rospy.Subscriber("/VR/joy_control", std_msgs.msg.Bool, self.call_back_control, queue_size=1)
        ...
        self.controller.initialize_robot_from_config(options)
        
        
        self.joystick = Joy()
        self.joystick.mode = "standing"
    ...
    def call_back_VR_joy_left(self, joy_msg:  sensor_msgs.msg.Joy):
        self.joystick.left_x = float(joy_msg.axes[0])
        self.joystick.left_y = float(joy_msg.axes[1])
        rospy.loginfo("Left x: " + str(self.joystick.left_x))
        rospy.loginfo("Left y: " + str(self.joystick.left_y))
        
    def call_back_VR_joy_right(self, joy_msg:  sensor_msgs.msg.Joy):
        self.joystick.right_x = float(joy_msg.axes[0])
        self.joystick.right_y = float(joy_msg.axes[1])
        rospy.loginfo("Right x: " + str(self.joystick.right_x))
        rospy.loginfo("Right y: " + str(self.joystick.right_y))
   
    def call_back_VR_mode(self, mode: std_msgs.msg.String):
        if mode.data in self.joystick.modes:
            self.joystick.mode = mode.data
            rospy.loginfo("Mode: " + str(self.joystick.mode))
        else: 
            rospy.loginfo("Invalid mode")
                    
    def call_back_stop(self, stop: std_msgs.msg.String):
        if stop.data == "stop":
            self.joystick.close()
            rospy.loginfo("Joystick closed")
            
    def call_back_control(self, control: std_msgs.msg.Bool):
        self.joystick.in_control = control.data
        rospy.loginfo("In control: " + str(self.joystick.in_control))

    def initialise_robot(self):
        self.controller.estop_buttons_pressed = True
        self.controller._toggle_estop()
        self.controller.estop_buttons_pressed = False
            
    def run(self,):
        self.initialise_robot()
        while not rospy.is_shutdown():
            try:
                if self.joystick.in_control:
                    rospy.loginfo("Controlling robot")
                    self.controller.control_robot(self.max_frequency, self.joystick)
                ## if we are not in control, we should not be controlling the robot                    
            except:
                self.controller._sit()
                self.controller._toggle_estop()
                break
            pass
...
def main():
    ...
    parser.add_argument('--max_frequency', default=10, type=int,
                        help='Max frequency in Hz to send commands to robot')
    parser.add_argument('--logging', default=True, action='store_true', help='Turn on logging output')
    parser.add_argument('--stairs_mode', help='Stairs mode [Y/n]', default="Y")
    # parser.add_argument('hostname', help="Hostname or address or robot")


    # bosdyn.client.util.add_base_arguments(parser)
    options, unknown = parser.parse_known_args()
    print("Unkowns: ", unknown)
    receiver = spot_receiver(options)
    
    try:
        rospy.loginfo("Starting run")
        receiver.run()
    except rospy.ROSInterruptException:
        rospy.signal_shutdown("Joystick closed")
