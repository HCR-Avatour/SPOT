...
class keyboard_teleop:
    ...
    def stand_service(self, key):
        ...
        if key=='j':
            ...
            tf.translation.z = self.height_down#crouch down
    ...
    def vel_service(self, key):
        ...
        if key=='w':
            twist.linear.x = self.lin_vel#forward/back
        elif key=='a':
            twist.linear.y = self.lin_vel#left/right
        elif key=='s':
            twist.linear.x = -1*self.lin_vel
        elif key=='d':
            twist.linear.y = -1*self.lin_vel
        elif key=='q':
            twist.angular.z = self.ang_vel#anticlockwise
        elif key=='e':
            twist.angular.z = -1*self.ang_vel#clockwise
...
if __name__ == "__main__":
    ...
    while not rospy.is_shutdown():
        ...
        if key in 'wasdqe':
            ...
            keyboard_teleop_ros.self_right_service(key)#lying down
        elif key in 'op':
